...
function syncLocation(replace){const q=new URLSearchParams();if(selectedView!=='overview')q.set('view',selectedView);if(selProjectId)q.set('project',selProjectId);if(selId)q.set('team',selId);if(selectedView==='conversations'&&selectedChannel)q.set('member',selectedChannel);const url=location.pathname+(q.toString()?'?'+q.toString():'');history[replace?'replaceState':'pushState'](null,'',url)}
...
function selectProject(id){selProjectId=id;const p=S?.projects?.find(p=>p.id===id);const t=(p?.teams||[]).filter(t=>t.status==='active').sort((a,b)=>b.timeUpdated-a.timeUpdated)[0]||(p?.teams||[])[0];if(t)selId=t.id;selectedChannel='@broadcast';selCard=-1;syncLocation(false);render()}
function selectTeam(id){selId=id;const t=cur();selProjectId=t?.projectId||selProjectId;if(selectedChannel!=='@broadcast'&&!(t?.members||[]).some(m=>m.name===selectedChannel))selectedChannel='@broadcast';selCard=-1;conversationError='';syncLocation(false);render()}
...
async function sendConversationMessage(event){event.preventDefault();if(conversationSending)return;const t=cur(),text=document.getElementById('conversation-text');if(!t||!text)return;const content=text.value.trim();if(!content){conversationError='Enter a message first.';render();return}conversationSending=true;conversationError='';render();try{await apiFetch('api/teams/'+encodeURIComponent(t.id)+'/messages',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({to:selectedChannel==='@broadcast'?null:selectedChannel,content:content})});text.value='';conversationPages.delete(conversationKey(t.id,selectedChannel));await ensureConversation(t.id,selectedChannel,false);conversationError='';}catch(err){conversationError=err.message||'Message could not be queued.'}finally{conversationSending=false;render();document.getElementById('conversation-text')?.focus()}}
...
addEventListener('popstate',function(){const q=new URLSearchParams(location.search);selectedView=q.get('view')==='conversations'?'conversations':'overview';selProjectId=q.get('project');selId=q.get('team');selectedChannel=q.get('member')||'@broadcast';conversationError='';render()});
