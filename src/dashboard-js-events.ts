...
function syncLocation(replace){const q=new URLSearchParams();if(selectedView!=='overview')q.set('view',selectedView);if(selProjectId)q.set('project',selProjectId);if(selId)q.set('team',selId);if(selectedView==='conversations'&&selectedChannel)q.set('channel',selectedChannel);const url=location.pathname+(q.toString()?'?'+q.toString():'');history[replace?'replaceState':'pushState'](null,'',url)}
...
addEventListener('popstate',function(){const q=new URLSearchParams(location.search);selectedView=q.get('view')==='conversations'?'conversations':'overview';selProjectId=q.get('project');selId=q.get('team');selectedChannel=q.get('channel')||(q.get('member')?'member:'+q.get('member'):'broadcast');conversationError='';render()});
